...
export class ServicesService {
  ...
  GetAllServices(): Observable<Service[]> {
    return this.http
      ...
      .pipe(
        map((services) =>
          services.filter((item) => item.priceForTheCurrentDay != null)
        )
      );
  }
      ...
      return this.http
        ...
        .pipe(
          map((services) =>
            services.filter(
              (item) =>
                item.category === catName && item.priceForTheCurrentDay != null
            )
          )
        );
    } else {
      ...
      return this.http
        ...
        .pipe(
          map((services) =>
            services.filter(
              (item) =>
                item.location === locName && item.priceForTheCurrentDay != null
            )
          )
        );
    } else {
      ...
      return this.http.get<Service[]>(this.API_URL).pipe(
        map((services) =>
          services.filter((item) => {
            ...
            return (
              price >= from && price <= to && item.priceForTheCurrentDay != null
            );
          })
        )
      );
      ...
      map((services) =>
        services
          .filter((item) => {
            const price = Number(item.priceForTheCurrentDay);
            return (
              (cat == null || cat == undefined || cat === item.category) &&
              (loc == null || loc == undefined || loc === item.location) &&
  ...
  GetRecomenditionServicesForBooking(): Observable<Service[]> {
    ...
    return forkJoin([bookingData$, serviceData$]).pipe(
      map(([bookingData, serviceData]) => {
        const serviceIds: number[] = bookingData.map((item) => item.serviceId);
        ...
        const filteredServices: Service[] = serviceData.filter((service) =>
          top8.includes(service.id)
        );
