    ...
    public getPropertyById(id: any) {
        this.appService.getPropertyById(id).subscribe((data) => {
            ...
            this.submitForm.controls.basic
                .get("title")!
                .setValue(this.property.title);
            this.submitForm.controls.basic
                .get("desc")!
                .setValue(this.property.desc);
            this.submitForm.controls.basic
                .get("priceDollar")!
                .setValue(
                    this.property.priceDollar.sale
                        ? this.property.priceDollar.sale
                        : this.property.priceDollar.rent
                );
            this.submitForm.controls.basic
                .get("priceEuro")!
                .setValue(
                    this.property.priceEuro.sale
                        ? this.property.priceEuro.sale
                        : this.property.priceEuro.rent
                );
            this.submitForm.controls.basic
                .get("propertyType")!
                .setValue(
                    this.propertyTypes.filter(
                        (p) => p.name == this.property.propertyType
                    )[0]
                );
            ...
            this.submitForm.controls.address
                .get("location")!
                .setValue(this.property.formattedAddress);
            this.lat = this.property.location.lat;
            this.lng = this.property.location.lng;
            this.getAddress();

            this.submitForm.controls.additional
                .get("bedrooms")!
                .setValue(this.property.bedrooms);
            this.submitForm.controls.additional
                .get("bathrooms")!
                .setValue(this.property.bathrooms);
            this.submitForm.controls.additional
                .get("garages")!
                .setValue(this.property.garages);
            this.submitForm.controls.additional
                .get("area")!
                .setValue(this.property.area.value);
            this.submitForm.controls.additional
                .get("yearBuilt")!
                .setValue(this.property.yearBuilt);
            ...
            this.property.videos.forEach((video) =>
                videos.push(this.fb.group(video))
            );

            const plans = this.submitForm.controls.media.get(
                "plans"
            ) as FormArray;
            while (plans.length) {
                plans.removeAt(0);
            ...
            this.property.plans.forEach((plan, index) => {
                ...
                this.submitForm.controls.media
                    .get("plans")!
                    ["controls"][index].controls.image.setValue([image]);
            });

            const additionalFeatures = this.submitForm.controls.media.get(
                "additionalFeatures"
            ) as FormArray;
            while (additionalFeatures.length) {
                additionalFeatures.removeAt(0);
            }
            this.property.additionalFeatures.forEach((feature) =>
                additionalFeatures.push(this.fb.group(feature))
            );

            this.submitForm.controls.media
                .get("featured")!
                .setValue(this.property.featured);
        });
    }
    ...
    public setAddresses(result) {
        ...
        result.address_components.forEach((item) => {
            if (item.types.indexOf("locality") > -1) {
                if (
                    this.cities.filter((city) => city.name == item.long_name)[0]
                ) {
            ...
            result.address_components.forEach((item) => {
                if (item.types.indexOf("neighborhood") > -1) {
                    let neighborhood = this.neighborhoods.filter(
                        (n) =>
                            n.name == item.long_name && n.cityId == newCity.id
                    )[0];
                    if (neighborhood) {
            ...
            result.address_components.forEach((item) => {
                if (item.types.indexOf("route") > -1) {
                    if (
                        this.streets.filter(
                            (street) =>
                                street.name == item.long_name &&
                                street.cityId == newCity.id
                        )[0]
                    ) {
