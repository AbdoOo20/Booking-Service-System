...
export class PagesComponent implements OnInit {
    ...
    @HostListener("window:scroll") onWindowScroll() {
        const scrollTop = Math.max(
            ...
            this.domHandlerService.winDocument.documentElement.scrollTop,
        ...
        if (this.settings.stickyMenuToolbar) {
            let top_toolbar =
                this.domHandlerService.winDocument.getElementById(
                    "top-toolbar"
                );
            if (top_toolbar) {
        ...
        let load_more =
            this.domHandlerService.winDocument.getElementById("load-more");
        if (load_more) {
            if (
                this.domHandlerService.window?.innerHeight >
                load_more.getBoundingClientRect().top + 120
            ) {
    ...
    public scrollToTop() {
        ...
        var scrollStep =
            -this.domHandlerService.window?.pageYOffset / (scrollDuration / 20);
